...
public class IntHistogram implements Histogram<Integer> {
    ...
    public void addValue(Integer v) {
        ...
        int bucketsIndex = (v - this.minVal) / this.width;
        if(bucketsIndex >= this.buckets){
            return;
        }
        this.totalTuples ++;
        this.heights[bucketsIndex]++;
    }
    ...
    private double estimateGreater(int bucketIndex,int predicateValue,int bucketWidth){
        ...
        double res = bucketRatio * (this.heights[bucketIndex] * 1.0 / this.totalTuples);

        int sum = 0;
        for(int i = bucketIndex;i<this.buckets ;i++){
            sum += this.heights[i];
        }
        return (sum * 1.0) / this.totalTuples +res;
    }
    ...
    private double estimateEqual(int bucketIndex,int predicateValue,int bucketWidth){
        ...
        double res = this.heights[bucketIndex];
        res = res / bucketWidth;
        res = res / this.totalTuples;
        return res;
    }
